...
def importFile(file_path,fileType):
    ...
    if fileType == "Discover":
        ...
        df["Amount"] = df["Amount"].apply(lambda x: x*-1 if x > 0 else x*-1)
    ...
    if fileType == "Ally Checking" or fileType == "Ally Savings":
        ...
        df["Date"]=pd.to_datetime(df.loc[:,"Date"])
        df["Category"]="Reset"
        if fileType == "Ally Checking":
            df["Source"]="Ally Checking"
        else:
             df["Source"]="Ally Savings"
...
def createSupportFiles():
    descDf=pd.read_csv("helper/Files/desc_mapping.csv")
    desc_mapping=dict(zip(descDf['Term'].str.upper(), descDf['Desc']))
    catDF=pd.read_csv("helper/Files/categoryLookup.csv")
    category_dict = dict(zip(catDF['Desc'].str.upper(), catDF['Category']))
    catStatDF=pd.read_csv("helper/Files/categoryStatusLookup.csv")
